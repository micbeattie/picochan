...
pch_schib_t __time_critical_func(*pop_ua_slist_unsafe)(ua_slist_t *l, pch_chp_t *chp) {
	int16_t head = l->head;
	if (head == -1)
		return NULL;
...
bool __time_critical_func(push_ua_slist_unsafe)(ua_slist_t *l, pch_chp_t *chp, pch_sid_t sid) {
	...
	if (tail == -1) {
		...
		was_empty = true;
                ...
                pch_schib_t *tail_schib = get_schib_by_chp(chp, tail_ua);
